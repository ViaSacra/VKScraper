...
class VkScraper(object):
    ...
    def scrape(self, executor=concurrent.futures.ThreadPoolExecutor(max_workers=10)):
        ...
        credentials = pika.PlainCredentials('hui', 'znaet123')
        parameters = pika.ConnectionParameters('185.224.248.237',
                                   5672,
                                   '/',
                                   credentials)
        ...
        for username in self.usernames:
            ...
            try:
                ...
                if user_id:
                    self.get_photos(dst, executor, future_to_item, user_id, channel)
                    self.get_saved(dst, executor, future_to_item, user_id, channel)
                    self.get_videos(dst, executor, future_to_item, user_id, channel)
                    self.get_stories(dst, executor, future_to_item, user_id, channel)
    ...
    def download(self, item, channel, account, save_dir='./'):
        """Downloads the media file"""
        url = max(item['sizes'], key=lambda size: size['height'])['url']

        #Тут закидываем в очередь RebbitMQ
        channel.basic_publish(exchange='',
                  routing_key="vk_img",
                  body=url)

        account.fotoUrl = url
        s = json.dumps(account.__dict__)
    ...
    def get_photos(self, dst, executor, future_to_item, username, channel):
        ...
        for item in tqdm.tqdm(self.photos_gen(username), desc='Searching {0} for photos'.format(username),
            ...
            if self.is_new_media(item):
                future = executor.submit(self.download, item, channel, account, dst)
    ...
    def get_saved(self, dst, executor, future_to_item, username, channel):
        ...
        for item in tqdm.tqdm(self.saved_gen(username), desc='Searching {0} for saved pictures'.format(username),
            ...
            if self.is_new_media(item):
                future = executor.submit(self.download, item, channel, dst)
                future_to_item[future] = item
    ...
    def get_videos(self, dst, executor, future_to_item, username, channel):
    ...
    def get_stories(self, dst, executor, future_to_item, username, channel):
